...
def generate(eta,low,high,observation_to_interpret,sample_space,distance_flag=False):
    random_vector = np.random.uniform(-1,1,len(observation_to_interpret))
    ...
    for i in range(len(observation_to_interpret)):
        ...
        # if np.issubdtype(type(sample_space[i][1]), np.integer):
            # random_point[i] = random_point[i].astype(type(sample_space[i][1]))
...
def find(radius_eta,observations_n,binary_classifier,observation_to_interpret,sample_space):
...
if __name__ == '__main__':
    ...
    enemy_star = find(radius_eta,observations_n,binary_classifier,observation_to_interpret,sample_space)
